...
Codex plugin and marketplace metadata for Dokki's hosted MCP server.

## Codex Plugin: Dokki MCP

The `dokki-mcp` plugin exposes Dokki's hosted MCP endpoint to Codex with OAuth.
...
Focused skills are included for workspace, table, artifact, and file workflows so
Codex can discover the right tools instead of treating Dokki as document-only.
